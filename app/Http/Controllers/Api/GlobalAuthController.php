...
class GlobalAuthController extends Controller
{
     ...
     * Supports company_id (UUID), subdomain, or company identifier
     *
     * POST /api/auth/login-with-company
     * Body: { "company_id": "uuid|subdomain", "email": "user@example.com", "password": "..." }
     */
    public function loginWithCompany(Request $request)
    {
        ...
        // Find company by ID or subdomain
        $company = Company::on('mysql')
            ->where(function ($query) use ($request) {
                $query->where('id', $request->company_id)
                      ->orWhere('subdomain', $request->company_id);
            })
            ->where('status', Company::STATUS_ACTIVE)
            ->first();
        ...
        try {
            ...
            // Forget current tenant (will be set again when user accesses subdomain)
            Company::forgetCurrent();
