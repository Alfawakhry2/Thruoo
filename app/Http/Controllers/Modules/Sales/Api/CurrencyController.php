...
class CurrencyController extends Controller
{
    ...
    public function all($moduleId): JsonResponse
    {
        ...
        $currencies = Currency::where('module_id', $moduleId)
            ->active()
            ->orderBy('name')
            ->get();
     ...
     * Get single currency
     */
    public function show($moduleId, $currencyId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
     ...
     * Update currency
     */
    public function update($moduleId, Request $request, $currencyId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
        ...
        $currency = Currency::where('module_id', $moduleId)->find($currencyId);
        if (!$currency) {
            return response()->json([
                'success' => false,
                'message' => 'Currency not found',
     ...
     * Delete currency
     */
    public function destroy($moduleId, $currencyId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
     ...
     */
    public function convert($moduleId, Request $request): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
