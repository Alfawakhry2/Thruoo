...
class PaymentMethodController extends Controller
{
    ...
    public function all($moduleId): JsonResponse
    {
        ...
        $paymentMethods = PaymentMethod::where('module_id', $moduleId)
            ->active()
     ...
     */
    public function show($moduleId, $paymentMethodId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
     ...
     */
    public function update($moduleId, Request $request, $paymentMethodId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
        ...
        $paymentMethod = PaymentMethod::where('module_id', $moduleId)->find($paymentMethodId);
        if (!$paymentMethod) {
            return response()->json([
                'success' => false,
                'message' => 'Payment method not found',
     ...
     */
    public function destroy($moduleId, $paymentMethodId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
