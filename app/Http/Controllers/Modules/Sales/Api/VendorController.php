...
class VendorController extends Controller
{
    ...
    public function all($moduleId): JsonResponse
    {
        ...
        $vendors = Vendor::where('module_id', $moduleId)
            ->active()
            ->orderBy('name')
            ->get();
     ...
     * Get single vendor
     */
    public function show($moduleId, $vendorId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
     ...
     * Update vendor
     */
    public function update($moduleId, Request $request, $vendorId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
        ...
        $vendor = Vendor::where('module_id', $moduleId)->find($vendorId);
        if (!$vendor) {
            return response()->json([
                'success' => false,
                'message' => 'Vendor not found',
     ...
     * Delete vendor
     */
    public function destroy($moduleId, $vendorId): JsonResponse
    {
        $module = Module::find($moduleId);
        if (!$module) {
            return response()->json(['success' => false, 'message' => 'Module not found'], 404);
